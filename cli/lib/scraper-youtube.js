 ...
 * YouTube 视频信息抓取脚本
 * 
 * 使用 yt-dlp 获取 YouTube 视频的完整信息（元数据、字幕等）
 * 可选择同时下载视频
 * 
 * 前置条件:
 *   1. 安装 yt-dlp:
 ...
 * 
 *   2. 安装 yt-dlp-ejs（用于解决 YouTube JavaScript 挑战）:
 *      .venv\Scripts\pip.exe install yt-dlp-ejs
 * 
 *   3. 安装 Node.js（JavaScript 运行时，用于解决 YouTube 反爬虫挑战）
 * 
 *   4. 安装 ffmpeg（用于视频格式转换和修复，下载视频时推荐）:
 *      winget install ffmpeg
 * 
 * 使用方法:
 *   node scripts/scrapeYoutube.js <YouTube-URL> [options]
 * 
 * 选项:
 *   --output <file>               指定输出文件名（默认保存到 work_dir/scrape/youtube 目录）
 *   --pretty                      美化 JSON 输出
 *   --download                    同时下载视频
 *   --audio-only                  仅下载音频（需配合 --download）
 *   --sub-langs <langs>           字幕语言（默认 zh-Hans,zh-Hant,en）
 *   --no-subtitles                不获取字幕
 ...
 * 
 * 文件保存位置:
 *   - 视频信息: work_dir/scrape/youtube/{VIDEO_ID}/{VIDEO_ID}.json
 *   - 下载的视频: work_dir/scrape/youtube/{VIDEO_ID}/{VIDEO_ID}.{ext}
 * 
 * 示例:
 *   node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID
 *   node scripts/scrapeYoutube.js https://www.youtube.com/shorts/VIDEO_ID
 *   node scripts/scrapeYoutube.js https://youtu.be/VIDEO_ID --pretty
 *   node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --download
 *   node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --download --audio-only
 *   node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --sub-langs "zh-Hans,en"
 *   node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --cookies-from-browser chrome
 *   node scripts/scrapeYoutube.js --from-json VIDEO_ID.json --download
 *   node scripts/scrapeYoutube.js --from-json work_dir/scrape/youtube/VIDEO_ID.json --download --audio-only
 *   node scripts/scrapeYoutube.js --from-json VIDEO_ID.json --extract-subtitle
 *   node scripts/scrapeYoutube.js --from-json VIDEO_ID.json --extract-subtitle --subtitle-format txt
 */
...
async function getVideoInfo(url, options = {}) {
    ...
    // 启用 Node.js 运行时以解决 YouTube 的 JavaScript 挑战
...
async function downloadVideo(url, outputPath, options = {}) {
    ...
    if (options.audioOnly) {
        ...
        // 使用最佳可用格式（包括 HLS 格式）
        // 优先选择 DASH 格式（分离的视频+音频），可以绕过 HLS 403 问题
    ...
    try {
        ...
        if (!path.isAbsolute(jsonPath)) {
            ...
            if (!fs.existsSync(jsonPath)) {
                const workDir = path.join(process.cwd(), 'work_dir', 'scrape', 'youtube');
                ...
                if (jsonBaseName.match(/^[a-zA-Z0-9_-]{11}$/)) {
...
async function main() {
    ...
    if (!options.url && !options.fromJson && !options.extractSubtitle) {
        console.error('错误: 请提供 YouTube 视频 URL 或使用 --from-json 选项，或使用 --extract-subtitle 从视频文件提取字幕');
        console.log('\n使用方法:');
        console.log('  node scripts/scrapeYoutube.js <YouTube-URL> [options]');
        console.log('  node scripts/scrapeYoutube.js --from-json <JSON文件> [options]');
        console.log('  node scripts/scrapeYoutube.js --from-json <JSON文件> --extract-subtitle');
        console.log('\n选项:');
        console.log('  --output <file>               指定输出文件名（默认保存到 work_dir/scrape/youtube 目录）');
        console.log('  --pretty                      美化 JSON 输出');
        console.log('  --download                    同时下载视频');
        console.log('  --audio-only                  仅下载音频（需配合 --download）');
        console.log('  --sub-langs <langs>           字幕语言（默认 zh-Hans,zh-Hant,en）');
        ...
        console.log('\n示例:');
        console.log('  node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID');
        console.log('  node scripts/scrapeYoutube.js https://www.youtube.com/shorts/VIDEO_ID');
        console.log('  node scripts/scrapeYoutube.js https://youtu.be/VIDEO_ID --pretty');
        console.log('  node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --download');
        console.log('  node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --download --audio-only');
        console.log('  node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --sub-langs "zh-Hans,en"');
        console.log('  node scripts/scrapeYoutube.js https://www.youtube.com/watch?v=VIDEO_ID --cookies-from-browser chrome');
        console.log('  node scripts/scrapeYoutube.js --from-json VIDEO_ID.json --download');
        console.log('  node scripts/scrapeYoutube.js --from-json work_dir/scrape/youtube/VIDEO_ID.json --download --audio-only');
        console.log('  node scripts/scrapeYoutube.js --from-json VIDEO_ID.json --extract-subtitle');
        console.log('  node scripts/scrapeYoutube.js --from-json VIDEO_ID.json --extract-subtitle --subtitle-format txt');
        process.exit(1);
    }
    ...
    if (options.extractSubtitle) {
        if (!options.fromJson) {
            ...
            console.log('示例: node scripts/scrapeYoutube.js --from-json VIDEO_ID.json --extract-subtitle');
            process.exit(1);
        }
        ...
        try {
                ...
                try {
                    ...
                    for (const file of files) {
                        ...
                        if (fileBaseName.toLowerCase() === videoIdForSubtitle.toLowerCase() && 
                            ['.mp4', '.webm', '.mkv', '.flv', '.m4a', '.mp3'].includes(path.extname(file).toLowerCase())) {
            ...
            if (!subtitleOutput) {
                // 创建以视频 ID 命名的子目录
                const normalizedVideoId = videoIdForSubtitle;
            ...
            try {
                ...
                const ytdlpCmd = getYtDlpCommand();
                const tempDir = path.join(os.tmpdir(), `yt-subs-${videoIdForSubtitle}-${Date.now()}`);
                
                try {
                    await fsPromises.mkdir(tempDir, { recursive: true });
                    
                    const defaultSubLangs = options.subLangs || 'zh-Hans,zh-Hant,en';
    ...
    // 统一使用视频 ID 作为子目录名
    const workDir = path.join(process.cwd(), 'work_dir', 'scrape', 'youtube');
    const videoSubDir = path.join(workDir, videoId);
